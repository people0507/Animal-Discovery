                        ...
                        <table id="multi_col_order" class="table table-striped table-bordered display "
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th style="text-align: center; vertical-align: middle;">Nội dung câu hỏi</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                                ...
                                                <a href="#"class="icon-action" data-toggle="modal"
                                                    data-target="#editQuestionModal_{{ $question->id }}"><i class="icon-settings"></i></a>
                                            </td>
                                    </tr>
                                @endforeach
                            </tbody>
...
@foreach ($questions as $key => $question)
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <form action="{{ route('admin.delete_question', ['id' => $question->id,'topic_id' =>$id]) }}" method="POST">
                            @csrf
        ...
        <div class="modal fade" id="editQuestionModal_{{ $question->id }}" tabindex="-1" role="dialog" aria-labelledby="editQuestionModal_{{ $question->id }}"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content" style=" padding:20px; border-radius: 12px;">
                ...
                <div class="modal-body">
                    <form action="{{route('admin.update_question_answer',['id' => $question->id])}}" id="editQuestionForm_{{ $question->id }}" enctype="multipart/form-data" method="post">
                        @csrf
                        ...
                        <div class="mb-2">
                                <input type="text" name="answer_content[]"   placeholder="Nhập nội dung" style="width: 75%;" value="{{$item->answer_content}}" required>
                                <input type="text" value="{{$item->id}}" hidden name="hidden_id[]">
                                <span class="ml-5">
                                    <input type="radio" name="answer_status"  value="{{$key}}"  {{$item->answer_status == 1 ? 'checked' :''}} required>
                                </span>
                        </div>
                        @endforeach
                        </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-primary" form="editQuestionForm_{{ $question->id }}">Cập Nhật</button>
                        </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="addQuestionModal" tabindex="-1" role="dialog" aria-labelledby="addQuestionModal"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content" style=" padding:20px; border-radius: 12px;">
                ...
                <div class="modal-body">
                    <form action="{{route('admin.create_question_answer',['id' => $id])}}" id="addQuestionForm" enctype="multipart/form-data" method="post">
                        @csrf
                        <div class="form-group">
                            <label for="questionTitle">Nội dung câu hỏi</label>
                        ...
                        <div  class="form-group">
                            <label for="questionTitle">Nội dung câu đáp án</label>
                        <div class="mb-2">
                                <input type="text" name="answer_content[]"   placeholder="Nhập nội dung" style="width: 75%;" required>
                                <span class="ml-5">
