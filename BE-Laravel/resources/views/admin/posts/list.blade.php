                            ...
                            <table id="multi_col_order" class="table table-striped table-bordered display no-wrap"
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{$post->title}}</td>
                                        <td><img src="http://127.0.0.1:8000/posts/{{$post->image}}" alt="{{$post->title}}" width="200"></td>
                                        <td style="width: 30%">{{$post->content}}</td>
                                        <td>{{$post->user->name}}</td>
                                        <td>Thích: {{$post->likes_count}},Bình Luận: {{$post->comments_count}}</td>
                                        @if($post->status == 0)
                                        ...
                                        <td>
                                            <a href="#" class="icon-action delete-action" data-toggle="modal"
                                                data-target="#deleteConfirmationModal_{{$post->id}}" style="color: red;">
                                                <i class="icon-close"></i>
                                            </a>
                                            @if($post->status == 0)
                                            /
                                            <a href="#" class="icon-action approve-action" data-toggle="modal"
                                            data-target="#approvalConfirmationModal_{{$post->id}}" style="color: green;">
